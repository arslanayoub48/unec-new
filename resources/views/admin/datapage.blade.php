                    ...
                    <h3>{{$params["title"]}}
                        @if(isset($params["translate"]) && $params["translate"])
                            @foreach(\App\Models\Wlang::all() as $lang)
                                <a class="language_links @if(\App\Models\Wlang::getCurrent() == $lang->slug) language_links_active @endif"
                                   href="?lang={{$lang->slug}}">{{$lang->name}}</a>
                            @endforeach
        ...
        <!-- top tiles -->
            <div class="row tile_count">
                ...
                <div class="col-md-2 col-sm-4 col-xs-6 tile_stats_count">
                    ...
                    <div class="count green">{{floatval($accounts->sum("personal") + $accounts->sum("expenses"))}}₼
                    </div>
                </div>
                <div class="col-md-2 col-sm-4 col-xs-6 tile_stats_count">
                ...
                <div class="col-md-2 col-sm-4 col-xs-6 tile_stats_count">
                    ...
                    <div class="count">{{floatval($accounts->sum("total")) - (floatval($accounts->sum("personal") + $accounts->sum("expenses")))}}
                        ₼
                    </div>
                </div>
                            ...
                            <tr style="height: 100px;">
                                @for($i=0;$i<count($data); $i++)
                                    @if(isset($params["imagecol"]) && $params["imagecol"] == $i)
                                        <td><a href="/dataPageAction?action=edit&id={{$data[0]}}"><img
                                                        src="/image/{{$data[$i]}}" style="width: 150px;"/></a></td>
                                    @elseif($i == 0)
                                                ...
                                                @foreach($params["actions"] as $action)
                                                    ...
                                                    <li>
                                                        @if(!strpos($action['link'], "remove"))
                                                            <a @if(isset($action["newtab"]) && $action["newtab"]) target="_blank"
                                                               @endif href="{{$action['link']}}&id={{$data[0]}}">
                                                                <i class="{{$action['icon']}}"></i>
                                                                {{$action["text"]}}
                                                            </a>
                                                        @else
                                                            <a href="#"
                                                               onclick="removeElement('{{$action['link']}}&id={{$data[0]}}')">
                                                                <i class="{{$action['icon']}}"></i>
                                                                {{$action["text"]}}
                                                            </a>
                                                        @endif
            ...
            <div class="modal-content">
                <form id="my_form" method="POST">
                        ...
                        <div class="container-fluid">
                            <div class="form-group">
                                ...
                                <select  class="form-control tagsselect"
                                        name=" <?php if ($params["title"] == "Xəbərlər") {
                                            echo "news";
                                        } if ($params["title"] == "Tədbirlər") {
                                            echo "events";
                                        } ?>tags[]" ata-role="tagsinput"
                                        multiple="multiple" required>

                                    <?php if($params["title"] == "Xəbərlər"){  ?>
                                    @foreach(getNewsTags('news') as $news_tag)
                                        <option value="{{$news_tag}}"> {{$news_tag}}</option>

                                    @endforeach
                                    <?php  }
                                    if($params["title"] == "Tədbirlər"){
                                    ?>
                                    @foreach(getNewsTags('events') as $news_tag)
                                        <option value="{{$news_tag}}"> {{$news_tag}}</option>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-primary" data-dismiss="modal" onclick="sendForm()" >Save</button>
                    </div>
                </form>
            </div>
        </div>
